...
function Store({ token, user, onLogout }) {
  ...
  const handleBuyData = async () => {
    ...
    setBusy(true);
    try {
      const data = await api.buyData(token, { planId: selectedPlan.id, phone: phone.trim() });
      setBalanceKobo(data.wallet_balance);
  ...
  return (
    <div className="min-h-screen bg-[#F4F7FB] text-[#111827]">
      ...
      <main className="px-5 max-w-md mx-auto pb-16 pt-3"><div className="grid grid-cols-2 gap-3 mb-6">
          <button
      ...
      {topUpOpen && (
        <div className="fixed inset-0 bg-black/40 flex items-end sm:items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-5 w-full max-w-sm border border-[#E5E9F0]">
            ...
            <div className="mb-5 pb-5 border-b border-[#E5E9F0]">
              ...
              {dedicatedAccount ? (
                <div className="bg-[#F0FDF4] border border-[#86EFAC] rounded-xl p-3.5">
                  <div className="text-xs text-[#16A34A] mb-1">{dedicatedAccount.bank}</div>
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-lg tracking-wide">{dedicatedAccount.account_number}</span>
                    <button
                      onClick={() => {
                    ...
                    >
                      <Copy size={16} className="text-[#16A34A]" />
                    </button>
                  </div>
                  <div className="text-xs text-[#6B7280] mt-1">{dedicatedAccount.account_name}</div>
                  <div className="text-[11px] text-[#6B7280] mt-2">
                    Transfer anytime — your wallet is credited automatically.
                  </div>
                </div>
              ) : (
                <button
                  ...
                  className="w-full rounded-lg py-2.5 text-xs font-semibold border border-[#2563EB] text-[#2563EB] disabled:opacity-50"
                >
                  {dedicatedLoading ? "Setting up your account…" : "Get my permanent account number"}
                </button>
              )}
            </div>

            <div className="text-[11px] uppercase tracking-widest text-[#6B7280] mb-2">
              Or pay now with card
            </div>
            <div className="flex gap-2 mb-3">
